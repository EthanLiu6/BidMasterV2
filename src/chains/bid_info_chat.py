...
from src.recall.search_knowledge import dense_biaodewu_search
...
def bid_info_chat(
        ...
        query: str,
        _collection_name: str = 'biaodewu_info_collection',
        thinking: bool = True
) -> str:
    query_emb, _ = sentence_model.encode(query)
    searched_res = dense_biaodewu_search(milvus_client, query_emb.tolist(), _collection_name)
    res_bid_info_texts = [each_res["entity"]["biaodewu_info_text"] for each_res in searched_res[0]]

    bid_info_prompt = BidInfoPrompt().build_prompt(
        question=query,
...
if __name__ == '__main__':
    ...
    while 1:
        user_query = input("请输入你咨询的招投标信息相关问题(q退出):\n")
        if user_query == 'q':
            break

        answer = bid_info_chat(qwen3, stc_model, client, user_query, thinking=False)
        print(answer)
